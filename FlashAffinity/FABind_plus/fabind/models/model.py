...
class FABindPlus(torch.nn.Module):
    ...
    def inference(self, data):
        ...
        keepNode_list = []  # Store keepNode information for each sample
        ...
        for i in range(pred_pocket_center.shape[0]):
            ...
            keepNode_list.append(keepNode)  # Store keepNode for this sample
        ...
        pocket_coords_out = self.unnormalize_coord(pocket_coords_out) + pocket_center_bias[pocket_batch]
        ...
        if self.confidence_training:
            if self.args.stack_mlp:
                ...
                confidence_hidden = scatter_add(src=complex_out, index=complex_batch, dim=0)
            confidence_score_pred = self.ranking_score_mlp(confidence_hidden).squeeze(-1)
            return compound_coords_out, compound_batch, confidence_score_pred, pocket_coords_out, pocket_batch, keepNode_list
        else:
            return compound_coords_out, compound_batch, pocket_coords_out, pocket_batch, keepNode_list
