...
def extract_torchdrug_feature_from_mol(mol, has_LAS_mask=False, ):
    ...
    assert edge_weight.max() == 1 if edge_weight.numel() > 0 else True
    assert edge_weight.min() == 1 if edge_weight.numel() > 0 else True
...
def generate_conformation(mol):
    ...
    try:
        ...
        if rid == -1:  # failed to embed
            # If embedding fails, use random coordinates
            ps.useRandomCoords = True
            rid = AllChem.EmbedMolecule(mol, ps)
            
        try:
            # Try MMFF optimization first
            AllChem.MMFFOptimizeMolecule(mol, confId=rid, maxIters=1000)
        except:
            # If MMFF fails, try UFF optimization (more tolerant)
            try:
                AllChem.UFFOptimizeMolecule(mol, confId=rid, maxIters=1000)
            except:
                pass

    except:
        raise Exception("Failed to generate conformation")
    mol = Chem.RemoveHs(mol)
    return mol
