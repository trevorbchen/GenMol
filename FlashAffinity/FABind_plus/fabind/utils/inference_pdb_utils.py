...
def get_protein_structure(res_list):
    ...
    # Original backbone coordinates (for compatibility)
    coords = []
    # New: full atom coordinates (heavy atoms only, no hydrogens)
    full_atom_coords = []
    
    def is_hydrogen(atom_name):
        """Check if atom is hydrogen based on naming conventions"""
        atom_name = atom_name.strip()
        return atom_name.startswith('H') or atom_name.startswith(('1H', '2H', '3H'))
    ...
    for res in res_list:
        ...
        # Full atom coordinates (heavy atoms only)
        res_full_atoms = {}
        for atom in res:
            atom_name = atom.name
            if not is_hydrogen(atom_name):  # Filter out hydrogen atoms
                res_full_atoms[atom_name] = list(atom.coord)
        full_atom_coords.append(res_full_atoms)
    
    structure['coords'] = coords  # Keep compatibility
    structure['full_atom_coords'] = full_atom_coords  # Add full atom information (heavy atoms only)
    return structure
