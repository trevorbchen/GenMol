...
class InferenceDataset(Dataset):
    def __init__(self, index_csv, pdb_file_dir, preprocess_dir):
        ...
        # Load unified molecule data
        processed_molecule_path = os.path.join(preprocess_dir, 'processed_molecule.pt')
        if os.path.exists(processed_molecule_path):
            print(f"Loading unified molecule data from {processed_molecule_path}")
            self.processed_molecules = torch.load(processed_molecule_path)
            print(f"Loaded {len(self.processed_molecules)} processed molecules")
        else:
            print(f"Warning: Unified molecule file not found at {processed_molecule_path}")
            print("Falling back to individual molecule files...")
            self.processed_molecules = None
        ...
        for i in tqdm(range(len(info))):
            ...
            try:
                ...
                if self.processed_molecules is not None and ligand_id in self.processed_molecules:
                    mol, molecule_info = self.processed_molecules[ligand_id]
                else:
                    raise ValueError(f'Molecule {ligand_id} not found in preprocessed molecules')
            except Exception as e:
                print(f'\nFailed to read molecule id {i} (ligand_id: {info.iloc[i].ligand_id}). Skipping it. Error: {e}')
                continue
        ...
        print(f"Successfully loaded {len(self.data)} protein-ligand pairs")
